...
const Navbar = () => {
  ...
  return (
    <nav className="fixed w-full bg-[#11baf0] bg-opacity-95 text-white h-14 lg:px-[5%] flex flex-wrap justify-between lg:gap-6 overflow-hidden shadow-lg duration-500">
      ...
      <aside className="w-full lg:w-fit lg:h-14 grow px-[5%] lg:px-0 flex flex-wrap justify-center lg:gap-8">
        {routes.map((e, index) => (
          <NavLink
            key={index}
            to={e.url}
      ...
      <div className="h-14 px-[5%] lg:px-0 flex items-center gap-4">
        <button
          ...
          className="w-28 py-1 block font-bold text-center border-2 border-white rounded hover:bg-white hover:text-[#11baf0]"
        >
          تسجيل الدخول
        </button>
        <button
